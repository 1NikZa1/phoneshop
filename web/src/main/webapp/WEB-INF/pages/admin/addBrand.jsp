    ...
    <div class="container mt-3">
        ...
        <div class="clearfix mt-1 mb-3">
            <a href="${pageContext.request.contextPath}/admin/brands"
               class="btn btn-success float-start">Back to brand list</a>
        </div>
        ...
        <div>
            <form:form id="add" method="post" modelAttribute="request" cssStyle="width: 350px">
                ...
                <tags:editFormRow name="name" label="Name" errors="${errors}"/>

                <button form="add" type="submit" class="btn btn-success float-end">
                    <c:if test="${not isNewBrand}">
                        Update
                    </c:if>
                    <c:if test="${isNewBrand}">
                        Save
                    </c:if>
                </button>
                <c:if test="${brandId != null}">
                    <a href="${pageContext.request.contextPath}/admin/deleteBrand/${brandId}" class="btn btn-success float-end">
                        Delete
                    </a>
                </c:if>
            </form:form>
